...
samples <- read.csv('/Users/LoboM/Dropbox/chip_bams/samplesheet_2.csv')
...
tmp1 = normalize.quantiles(as.matrix(matrix_counts_tmm[4:11]))
rownames(tmp1) = paste(paste(matrix_counts_tmm$CHR,matrix_counts_tmm$START,sep=':'),matrix_counts_tmm$END,sep='-')
colnames(tmp1) = colnames(matrix_counts_tmm)[4:11]
write.table(tmp1, file = file.path(savedir,"consensus_atac_H3K27ac_counts_tmm_quan_norm.csv"), sep = "\t",
            row.names = TRUE, col.names = TRUE, quote=FALSE)
write.table(matrix_counts_tmm, file = file.path(savedir,"consensus_atac_H3K27ac_counts_tmm.csv"), sep = "\t",
            row.names = TRUE, col.names = TRUE, quote=FALSE)
